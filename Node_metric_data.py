...
def process_node(node, writer, lock):
    ...
    row_data = {"Timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Node Name": node["name"]}
    ...
    # Check if any data was returned for the node
    if len(row_data) > 2:  # Timestamp and Node Name are always present
        with lock:
            writer.writerow(row_data)
    else:
        error_message = f"No data returned for {node['host']}"
        print(error_message)
        with lock:
            with open("error.log", "a") as error_log:
                error_log.write(error_message + "\n")
...
with open(output_file, mode="w", newline="") as file:
    ...
    threads = []
    for node in nodes:
        thread = threading.Thread(target=process_node, args=(node, writer, lock))
